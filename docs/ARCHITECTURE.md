...
seohead/
  ...
  sf/                 CRAWL AUDIT (Screaming Frog)
    ...
    core/             loader -> context -> rules (registry, 114 checks)
...
SF exports (mode B) or SF CLI crawl (mode A)
    ...
    -> rules/inlinks/heuristics/sitemap (checks, 114 ids in registry.py)
...
672 tests, **all offline**; runtime varies by Python version and installed extras. See
