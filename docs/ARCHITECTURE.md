...
seohead/
  ...
  sf/                 CRAWL AUDIT (Screaming Frog)
    ...
    core/             loader -> context -> rules (registry, 118 checks)
...
SF exports (mode B) or SF CLI crawl (mode A)
    ...
    -> rules/inlinks/heuristics/sitemap (checks, 118 ids in registry.py)
...
over 1100 tests, **all offline**; runtime varies by Python version and installed extras. See
