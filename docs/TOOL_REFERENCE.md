...
Turn an audit document into a file: xlsx, docx, csv, md or json. Pass the dict returned by seo_site_audit (or a path to its JSON). xlsx has four sheets with filters and a live Excel chart — for work; docx is prose with headings — for the client; csv is flat data for a tracker (two files: findings and pages); md is for reading and for git. The generators compute nothing and reach no network: what is not in the JSON does not appear in the report.
